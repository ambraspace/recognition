...
public class Stage2 {
	...
	private static final Logger log = LoggerFactory.getLogger(Stage2.class);
	...
	private static class FileStack {
		...
		public synchronized void addFileToStack(File f) {
			files.add(f);
		}
		
		public synchronized File pullFileFromStack() {
			if (files.isEmpty()) {
				return null;
			}
			File retVal = files.remove(0);
			log.info("Processing file #" + fileCounter + ": " + retVal.getAbsolutePath());
			fileCounter++;
			return retVal;
		}
	...
	private static class NegativeSampleSearch implements Runnable {
		
		private FileStack fileStack;
		private File currentFile;
		
		public NegativeSampleSearch(FileStack f) {
			this.fileStack = f;
		}
		...
		public void run() {
			...
			currentFile = null;
			while ((currentFile=fileStack.pullFileFromStack())!=null) {
				processCurrentFile();
			}
		...
		private List<Frame> collectFrames(Mat image, int kernelSize, int stride) {
			...
			for (int i=0; i<rows; i++) {
				for (int j=0; j<cols; j++) {
					...
					try {
						INDArray input = new NativeImageLoader(13, 13, 1).asMatrix(frame);
						ipps.preProcess(input);
						INDArray result = detector.output(input);
						if (result.getDouble(1)>=0.001) {
...
//			return filteredFrames;
		...
		private double iOu(Frame f1, Frame f2) {
		...
		private void saveFrames(Mat image, List<Frame> frames) {
			...
			while (it.hasNext()) {
				...
				frame = image.colRange(f.x, f.x+f.w).rowRange(f.y, f.y+f.w);
				resize(frame, frame, new Size(25, 25));
		        if (frame.channels() != 1) {
		            int code = -1;
		            ...
		            if (code < 0) {
		                log.error("Cannot convert from " + frame.channels()
		                                                    + " to " + 1 + " channel.");
		            }
		            cvtColor(frame, frame, code);
		        }
				imwrite("/home/ambra/Deep Learning/LFW/training/detection-25/00/"+
						prefix+"-"+saveCounter+".png", frame);
				saveCounter++;
				frame.release();
				counter++;
				if (counter>10) break;
	...
	static final int minFaceSize = 25;

	static final int kernelSize = 25;
	...
	public static void main(String[] args) {
		...
		try {
			detector = ModelSerializer.restoreMultiLayerNetwork("/home/ambra/Deep Learning/LFW/training/detection-13/9943.zip", false);
			calibrator = ModelSerializer.restoreMultiLayerNetwork("/home/ambra/Deep Learning/LFW/training/calibration-13/7095.zip", false);
		...
		File srcDir = new File("/home/ambra/Deep Learning/ImageNet/class2/");
		...
		FileStack fstack = new FileStack();
		for (File f:files) {
			fstack.addFileToStack(f);
		}
		
		ExecutorService threadPool = Executors.newCachedThreadPool();
		
		for (int i=0; i<8; i++) {
			NegativeSampleSearch nss = new NegativeSampleSearch(fstack);
			threadPool.execute(nss);
		}
